...
1. Use React.Lazy to dynamically import modules later
   - `const Globe = React.lazy(() => import('../globe'));`
2. Wrap lazy components in React.Suspense, and provide a fallback UI
3. Eagerload modules/components on mouseover events
   - calling import multiple times has no effect, as webpack caches the modules
4. Webpack magic comments will auto add `<link rel='prefetch' as='script'>`  tags to lazy load modules
